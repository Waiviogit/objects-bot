...
const { accountsData, regExp, guestRequestsData } = require('constants/index');
...
const postBroadcaster = async (noMessageWait = 6000, postingErrorWait = 60000) => {
  ...
  if (message) {
    ...
    if (transactionStatus) {
      config.guest_comment.account === accountsData.guestOperationAccounts.length - 1
...
const commentBroadcaster = async (noMessageWait = 10000, postingErrorWait = 10000) => {
  ...
  if (message) {
    ...
    if (transactionStatus) {
      config.guest_comment.accountForComm === accountsData.guestOperationAccounts.length - 1
    ...
    (config.guest_comment.attemptsComm === (accountsData.guestOperationAccounts.length - 1)) {
